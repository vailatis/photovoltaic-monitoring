...
<?php PAGE_Open("Energia Settimanale", ""); ?>
            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div id="weekly-produzione" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div id="weekly-consumo" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div id="weekly-sorgenti" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
                    </div>
                </div>
            </div>
        </div>

<?php PAGE_Close("weekly_energycharts"); ?>
...
$dataCONSUMO = "";
$dataIMPORTATO = "";
$sqlCMD = "SELECT timestamp, consumo_wh, acc_wh, prod_pv_wh, import_wh FROM vw_inverter_power_daily WHERE timestamp >= date_format(now() - INTERVAL 6 DAY,'%Y%m%d') ORDER BY timestamp ASC;";
...
$tmpDATA = [0,0,0,0,0,0,0];
$tmpSOLARE = [0,0,0,0,0,0,0];
$tmpSOLBATT = [0,0,0,0,0,0,0];
$tmpBATTERIE = [0,0,0,0,0,0,0];
$tmpRETE = [0,0,0,0,0,0,0];
$CNT = -1;
$prevDATE = "";
$sqlCMD = "SELECT LEFT(timestamp,8) AS 'timestamp',IF (battery_discharge_current > 0 and status_code='G', 'H', status_code) AS 'status_code',COUNT(*) AS 'totalMinutes' FROM inverter_data WHERE timestamp >= date_format(now() - INTERVAL 6 DAY,'%Y%m%d0000') GROUP BY 1,2 ORDER BY 1 ASC";
$sqlRESULT = $sqlCONN->query($sqlCMD);
while ($sqlROW = $sqlRESULT->fetch_assoc()) {
   // Get query data
   $rawDATETIME = $sqlROW["timestamp"]."0000";
...
$(document).ready(function () {
   // Define the WEEKLY-PRODUZIONE graph object and its properties
   $('#weekly-produzione').highcharts({
      chart: {type: 'column'},
      title: {text: ''},
   ...
   // Define the WEEKLY-CONSUMO graph object and its properties
   $('#weekly-consumo').highcharts({
      chart: {type: 'column'},
      title: {text: ''},
